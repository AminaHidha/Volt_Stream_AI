...
from services.booking_agent_service import run_booking_agent

router = APIRouter()


@router.post("/agent/booking", response_model=AgentBookingResponse)
def agent_booking(request: AgentBookingRequest, authorization: str = Header(None)):
    if not authorization:
        raise HTTPException(status_code=401, detail="Missing Authorization header")

    today_str = date.today().isoformat()

    reply_text, booking_result = run_booking_agent(
